...
An _estimand_ (or _quantity of interest_) is some summary of parameters that we are interested in estimating
...
## Statistical significance 2 { .smaller }

::: { .callout-caution title="A common definition" }
- Statistical significance is conventionally defined as a threshold (commonly a $p$-value less than 0.05) relative to some _null hypothesis_ or _prespecified value_, which would indicate no effect present.

- E.g., an estimate may be considered "statistically significant at $P < 0.05$" if it:
  - lies at least two standard errors[^2] from the mean
...
[^2]: Remember the probability mass of the normal distribution?
...
- We measure the average post-treatment cholesterol levels $\bar{y}_T$ and $\bar{y}_C$ for the _treatment_ and _control_ groups as **estimates** for the true post-treatment levels $\theta_T$ and $\theta_C$.
:::
...
- We need to define a distribution for the null hypothesis
  - This allows us to define a test statistic (e.g. $T$ for a _t_-test)
