...
p2 <- ggplot(data %>% filter(str_detect(params, "^binom")), aes(x=count, color=params)) +
  ...
  scale_color_discrete(labels=c("n=20, p=0.01", "n=20, p=0.3", "n=20, p=0.7", "n=40, p=0.3")) +
  coord_fixed(ratio=50) +
  theme_minimal()
...
- All measured values are positive whole numbers or zero; means may be positive _real numbers_ or zero
...
- Statistical significance is conventionally defined as a threshold (commonly stated as a $p$-value less than 0.05) relative to some _null hypothesis_ or _prespecified value_ that indicates no effect is present.
...
::: { .callout-important title="This process relies on probability distributions" }
...
- To perform a statistical test, we need to assume a distribution and parameters for the _null hypothesis_
...
ggplot() +
  ...
  annotate("segment",                                          # show the observed difference as a dashed line
            x=-1.7, xend=-1.7, y=0, yend=0.3,
            colour="darkorange1", size=1, linetype="dashed") +
  annotate("text",
           x=-1.7, y=0.31,
           label="estimate = -1.7", color="darkorange3") +
  theme_minimal()
```
...
# Generate data to shade a normal curve
xmin = mu - 4 * std
xmax = mu + 4 * std
...
ggplot() +
  stat_function(fun=dnorm, args=list(mean=mu, sd=std), geom="line") +
  shade_curve(data, zstart=qnorm(0.05, mu, std), zend=qnorm(0.95, mu, std)) + 
  annotate("pointrange",
...
ggplot() +
  ...
  annotate("text",
           x=-1.7, y=0.31,
           label="estimate = -1.7, not significant", color="darkorange3") +
  annotate("segment",                                          # show the observed difference as a dashed line
            x=1.2, xend=1.2, y=0, yend=0.3,
            colour="darkorange1", size=1, linetype="dashed") +
  annotate("text",
           x=1.2, y=0.31,
           label="estimate = 1.2, not significant", color="darkorange3") +
    xlim(-4, 4) +
  annotate("segment",                                          # show the observed difference as a dashed line
            x=-2.1, xend=-2.1, y=0, yend=0.35,
            colour="purple", size=1, linetype="dashed") +
  annotate("text",
           x=-2.1, y=0.36,
           label="estimate = -2.1, significant", color="purple") +
  annotate("segment",                                          # show the observed difference as a dashed line
            x=3, xend=3, y=0, yend=0.35,
            colour="purple", size=1, linetype="dashed") +
  annotate("text",
           x=3, y=0.36,
           label="estimate = 3,  significant", color="purple") +
    xlim(-4, 4) +
  xlab(TeX("true difference, $\\theta$")) +
  ylab("density") +
  theme_minimal()
...
ggplot() +
  stat_function(fun=dnorm, args=list(mean=mu, sd=std), geom="line") +
  shade_curve(data, zstart=qnorm(0.05, mu, std), zend=qnorm(1, mu, std)) + 
#  annotate("pointrange",
#           xmin=qnorm(0.025, mu, std), xmax=qnorm(0.975, mu, std),
#           x=qnorm(0.5, mu, std), y=dnorm(qnorm(0.975, mu, std), mu, std), alpha=0.4) + 
#  annotate("text",
#           x=mu, y=dnorm(qnorm(0.975, mu, std), mu, std)-0.01,
#           label="95% CI") +
  annotate("segment",                                          # left limit
           x=qnorm(0.05, mu, std), xend=qnorm(0.05, mu, std),
           y=0, yend=dnorm(qnorm(0.05, mu, std), mu, std)) +
  annotate("text",
           x=qnorm(0.05, mu, std), y=-0.01,
           label="5%") +
#  annotate("segment",                                          # right limit
#           y=0, yend=dnorm(qnorm(0.975, mu, std), mu, std)) +
#           x=qnorm(0.975, mu, std), xend=qnorm(0.975, mu, std),
#  annotate("text",
#           x=qnorm(0.975, mu, std), y=-0.01,
#           label="97.5%") +  
  ...
  annotate("text",
           x=-1.7, y=0.31,
           label="estimate = -1.7, significant", color="purple") +
  annotate("segment",                                          # show the observed difference as a dashed line
            x=1.2, xend=1.2, y=0, yend=0.3,
            colour="darkorange1", size=1, linetype="dashed") +
  annotate("text",
  ...
  annotate("text",
           x=3, y=0.36,
           label="estimate = 3, not significant", color="darkorange3") +
    xlim(-4, 4) +
  xlab(TeX("true difference, $\\theta$")) +
  ylab("density") +
  theme_minimal()
...
ggplot() +
  stat_function(fun=dnorm, args=list(mean=mu, sd=std), geom="line") +
  shade_curve(data, zstart=qnorm(0, mu, std), zend=qnorm(0.95, mu, std)) + 
#  annotate("pointrange",
#           xmin=qnorm(0.025, mu, std), xmax=qnorm(0.975, mu, std),
#           x=qnorm(0.5, mu, std), y=dnorm(qnorm(0.975, mu, std), mu, std), alpha=0.4) + 
#  annotate("text",
...
#           label="5%") +
  ...
  annotate("text",
           x=-2.1, y=0.36,
           label="estimate = -2.1, not significant", color="darkorange3") +
  annotate("segment",                                          # show the observed difference as a dashed line
            x=3, xend=3, y=0, yend=0.35,
            colour="purple", size=1, linetype="dashed") +
  annotate("text",
...
1. Only a small change is required to move from $P < 0.051$ to $P < 0.049$
...
shade_curve = function(mu, std, zstart, zend, fill, alpha=0.5) {
    xmin = mu - 4 * std
    xmax = mu + 4 * std
    ...
    geom_area(data=subset(data, x >= zstart & x < zend ),
              aes(x=x, y=y),
              fill=fill, color=NA, alpha=alpha)
}

# mu=10, std=10, zstart=qnorm(0, 10, 10), zend=qnorm(0.95, 10, 10), fill="orange")
# Plot figurative null hypothesis distribution
ggplot() +
  stat_function(fun=dnorm, args=list(mean=10, sd=10), geom="line", colour="orange") +
  shade_curve(mu=10, std=10, zstart=qnorm(0.1, 10, 10), zend=qnorm(1, 10, 10), fill="orange") + 
  annotate("segment", colour="orange",
         x=qnorm(0.1, 10, 10), xend=qnorm(0.1, 10, 10),
         y=0, yend=dnorm(qnorm(0.1, 10, 10), 10, 10)) +
  annotate("text", colour="orange",
           x=qnorm(0.1, 10, 10), y=-0.001,
           label="10%") +  
  annotate("segment", colour="orange",
         x=qnorm(0.5, 10, 10), xend=qnorm(0.5, 10, 10),
         y=0, yend=dnorm(qnorm(0.5, 10, 10), 10, 10)) +
  annotate("text", colour="orange",
           x=qnorm(0.5, 10, 10), y=0.042,
           label="mean=10, sd=10") +  
  stat_function(fun=dnorm, args=list(mean=25, sd=10), geom="line", colour="purple") +
  shade_curve(mu=25, std=10, zstart=qnorm(0.01, 25, 10), zend=qnorm(1, 25, 10), fill="purple") + 
  annotate("segment", colour="purple",
         x=qnorm(0.01, 25, 10), xend=qnorm(0.01, 25, 10),
         y=0, yend=dnorm(qnorm(0.01, 25, 10), 25, 10)) +
  annotate("text", colour="purple",
           x=qnorm(0.01, 25, 10), y=-0.001,
           label="1%") +  
  annotate("segment", colour="purple",
         x=qnorm(0.5, 25, 10), xend=qnorm(0.5, 25, 10),
         y=0, yend=dnorm(qnorm(0.5, 25, 10), 25, 10)) +
  annotate("text", colour="purple",
           x=qnorm(0.5, 25, 10), y=0.042,
           label="mean=25, sd=10") +  
  annotate("segment", colour="black", lty="dashed",
         x=0, xend=0,
         y=0, yend=0.05) +
  xlim(-20, 60) +
  ylim(-0.001, 0.05) +
...
ggplot() +
  stat_function(fun=dnorm, args=list(mean=10, sd=10), geom="line", colour="orange") +
  shade_curve(mu=10, std=10, zstart=qnorm(0.025, 10, 10), zend=qnorm(0.975, 10, 10), fill="orange") + 
  annotate("segment", colour="orange",
         x=qnorm(0.025, 10, 10), xend=qnorm(0.025, 10, 10),
         y=0, yend=dnorm(qnorm(0.025, 10, 10), 10, 10)) +
  annotate("text", colour="orange",
           x=qnorm(0.025, 10, 10), y=-0.001,
           label="2.5%") +  
  annotate("segment", colour="orange",
         x=qnorm(0.975, 10, 10), xend=qnorm(0.975, 10, 10),
         y=0, yend=dnorm(qnorm(0.975, 10, 10), 10, 10)) +
  annotate("text", colour="orange",
           x=qnorm(0.975, 10, 10), y=-0.001,
           label="97.5%") +  
  annotate("segment", colour="orange",
         x=qnorm(0.5, 10, 10), xend=qnorm(0.5, 10, 10),
         y=0, yend=dnorm(qnorm(0.5, 10, 10), 10, 10)) +
  annotate("text", colour="orange",
           ...
           label="mean=10, sd=10") +  
  stat_function(fun=dnorm, args=list(mean=25, sd=10), geom="line", colour="purple") +
  shade_curve(mu=25, std=10, zstart=qnorm(0.025, 25, 10), zend=qnorm(0.975, 25, 10), fill="purple") + 
  annotate("segment", colour="purple",
         x=qnorm(0.025, 25, 10), xend=qnorm(0.025, 25, 10),
         y=0, yend=dnorm(qnorm(0.025, 25, 10), 25, 10)) +
  annotate("text", colour="purple",
           x=qnorm(0.025, 25, 10), y=-0.001,
           label="2.5%") +  
  annotate("segment", colour="purple",
         x=qnorm(0.975, 25, 10), xend=qnorm(0.975, 25, 10),
         y=0, yend=dnorm(qnorm(0.975, 25, 10), 25, 10)) +
  annotate("text", colour="purple",
           x=qnorm(0.975, 25, 10), y=-0.001,
           label="97.5%") +  
  annotate("segment", colour="purple",
         x=qnorm(0.5, 25, 10), xend=qnorm(0.5, 25, 10),
         y=0, yend=dnorm(qnorm(0.5, 25, 10), 25, 10)) +
